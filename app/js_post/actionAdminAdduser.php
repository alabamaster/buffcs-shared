...
switch ($case) {
	case '1': // сервер > привилегия
		...
		$sql = DB::run('SELECT * FROM `ez_privileges` WHERE `server_id` = ? ORDER BY `id`', [$server_id])->fetchAll();

		echo '<option value="0">Выберите привилегию</option>';
		foreach ($sql as $row) {
			$p_name = ($row['active'] == 1) ? $row['name'] : '[OFF] ' . $row['name'];
			echo '<option value="'.$row['id'].'">'.$p_name.'</option>';
		}
		break;
	...
	case '2': // привилегия > срок пирвилегии
		...
		$sql = DB::run('SELECT * FROM `bp_tarif_time` WHERE `tarif_id` = ? ORDER BY `price`', [$privilege_id])->fetchAll();
	...
	case '4': // описание привилегии
		$privilege_id = (int)$_POST['privilege_id'];
		$sql = DB::run('SELECT * FROM `editor` WHERE `tarif_id` = ?', [$privilege_id])->fetch(PDO::FETCH_ASSOC);
